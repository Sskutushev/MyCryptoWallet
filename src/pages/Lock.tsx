...
export const Lock = () => {
  ...
  const handleBiometric = async () => {
    ...
    // Эта функция может потребовать пароль в качестве запасного варианта

    // или для первоначальной настройки. Для простоты пока оставляем так.

    const success = await biometricAuth.authenticate()

    if (success) {

      await unlock('') // Предполагается, что биометрия разблокирует ключ
